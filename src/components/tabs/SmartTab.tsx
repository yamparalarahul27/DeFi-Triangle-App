      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs min-w-[720px]">
          <thead className="bg-surface-page text-fg-subtle">
            <tr>
              <th className="text-left font-medium uppercase tracking-wider px-4 py-3 sticky left-0 bg-surface-page z-10">Token</th>
            ...
            {sorted.map((pair, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right font-mono text-buy">{fmtNum(Number(pair?.txns?.h24?.buys ?? 0))}</td>
                  <td className="px-4 py-3 text-right font-mono text-sell">{fmtNum(Number(pair?.txns?.h24?.sells ?? 0))}</td>
                  <td className="px-4 py-3 text-right font-mono text-fg">{fmtUsd(Number(pair?.volume?.h24 ?? 0), { compact: true })}</td>
