...
// ── Order book — the Phase-5 gate demo. Fixed row heights on the
// density tokens + tabular numerals = ZERO layout shift while prices
// and depth stream. Flip the studio to compact and it tightens live.
    ...
    <div className="min-w-0 flex-1">
      {rows.map((r) => (
        ...
        >
          ...
          />
          <span className={`data-sm relative tabular-nums ${tone === "buy" ? "text-buy" : "text-sell"}`}>
            {r.px.toFixed(2)}
          </span>
...
function OrderBookDemo() {
  const [bids, setBids] = useState(() => mkSide(184.2, -1));
  const [asks, setAsks] = useState(() => mkSide(184.2, 1));
  useEffect(() => {
    const id = setInterval(() => {
      setBids((rows) => rows.map((r) => ({ ...r, qty: Math.max(20, Math.round(r.qty + (Math.random() - 0.5) * 60)) })));
      setAsks((rows) => rows.map((r) => ({ ...r, qty: Math.max(20, Math.round(r.qty + (Math.random() - 0.5) * 60)) })));
    }, 900);
    return () => clearInterval(id);
  }, []);
  ...
  return (
    <div className="rounded-card border border-outline-variant bg-surface-container">
      ...
      >
        <span>Bids</span>
        <span className="data-sm normal-case tabular-nums text-fg-muted">spread {spread.toFixed(2)}</span>
        <span>Asks</span>
      </div>
