...
- **Replace text-character keyboard hints with Lucide icons.** Three sites render shortcut keys as Unicode (`⌘K`, `↵`, `Esc`) inside `<kbd>` — [`src/components/search/HeroSearchButton.tsx`](src/components/search/HeroSearchButton.tsx#L20), [`src/components/layout/Header.tsx`](src/components/layout/Header.tsx#L144), [`src/components/search/SearchModal.tsx`](src/components/search/SearchModal.tsx#L385). Swap to `lucide-react` icons (`Command`, `CornerDownLeft`, etc.) for consistent visual language and cross-platform rendering — `⌘` looks broken on Windows.
